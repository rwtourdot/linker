#!/bin/bash

#CHR_list=('chr1' 'chr2' 'chr3' 'chr4' 'chr5' 'chr6' 'chr7' 'chr8' 'chr9' 'chr10' 'chr11' 'chr12' 'chr13' 'chr14' 'chr15' 'chr16' 'chr17' 'chr18' 'chr19' 'chr20' 'chr21' 'chr22' 'chrX')
CHR_list=('chr22' 'chr21' 'chr20' 'chr19' 'chr18' 'chr17' 'chr16' 'chr15' 'chr14' 'chr13' 'chr12' 'chr11' 'chr10' 'chr9' 'chr8' 'chr7' 'chr6' 'chr5' 'chr4' 'chr3' 'chr2' 'chr1' 'chrX')

#input_vcf_file="./sample_data/RPE-1.hets.vcf"
#input_bam_file="/czlab/Data/RPE-1_10X/RPE-1_bam/phased_possorted_bam.bam"
#input_vcf_file="./sample_data/RPE-1.hets.chr1-X.BA.SNP.rmcent.vcf"
#input_cov_file="./output/het_coverage_jan22_RPE1_illumina"
input_cov_file="./output/filtered_coverage_jan22_RPE1_tenx"
input_bam_file="/czlab/Data/RPE-1/bulk_10X/RPE-1_bulk.10X.bam"

tech="tenx"

string_id="jan22_filtered"
nohup_file="nohup_phase_"$string_id"_"$tech"_"


for chr_choice in "${CHR_list[@]}"
do
	input_cov_file_chr=$input_cov_file"_"$chr_choice".dat"
        loop_nohup_file=$nohup_file$chr_choice".out"
        echo $loop_nohup_file
        echo $string_id
        nohup ./linker phase -c $chr_choice -i $input_bam_file -m $input_cov_file_chr -e $tech -n $string_id > $loop_nohup_file
done



#input_vcf_file="./sample_data/K562_het_variants.vcf"
#input_bam_file="/czlab/Data/K-562-10X/bam1/phased_possorted_bam.bam"
#input_bam_file2="/czlab/Data/K-562-10X/bam2/phased_possorted_bam.bam"
#input_vcf_file="./sample_data/BL1954_PCRFree.hets.recalibrated.vcf"
