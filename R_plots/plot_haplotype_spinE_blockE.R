...
#chrom_list <- c("chr21")
hap_file <- "./../../../cpp_linker_v4.3/output/hap_solution_nov26_NA12878_v2_"
...
########################################
########################################
break_list_linspace = linspace(0, 25000, n = 1000)
...
p1_spinE <- ggplot(data=hap_tracks_spinE) + geom_line(aes(x=midpoint,y=count)) + #+ geom_freqpoly(aes(x=-spinE,y = ..count..),size=1.0,bins=90) +  #,center = 0 #pad = FALSE  #,na_rm=TRUE,inherit.aes = FALSE
  ...
  scale_x_continuous(expand = c(0.03,0.03),limits=c(0,4000)) + #2000  #3000  #limits=c(0,3000)  #expand = c(0.03,0.03) #expand = c(0.0,0.0)
...
p2_blockE <- ggplot(data=hap_tracks_blockE) + geom_line(aes(x=midpoint,y=count)) + #+ geom_freqpoly(aes(x=-blockE,y = ..count..),size=1.0,bins=90) + # ,binwidth = 1/4 #+ geom_histogram(aes(x=-blockE),bins=90) +  #,na_rm=TRUE,inherit.aes = FALSE
  ...
  geom_vline(aes(xintercept=1000),size=0.8,alpha = 0.8,linetype = "dotted") + #,color="blue" #"700" #geom_vline(aes(xintercept=100,color="red"),size=1.1,alpha = 0.5) + #"100"   #geom_vline(aes(xintercept=2000,color="green"),size=1.1,alpha = 0.5) + #"2000"
  scale_x_continuous(expand = c(0.03,0.03),limits=c(0,15000)) +
  ylab("Count") +
  xlab("blockE") +
  theme_classic() +
  theme(legend.position="none")
